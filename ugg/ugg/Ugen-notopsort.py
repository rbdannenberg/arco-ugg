...
#        print("ENTER get_ugen_list of: " + str(self))
    ...
    # and every successive value, as indicated by Rest(), which puts the
    # expression in rest.
...
#            print("adding", self, "to to_be_printed")
    ...
    # First is specified by both First() and Rest(), indicating initialization and
    # each successive value, computed at audio or block rate
    def get_all_parameters(self):
        if not self.rest:
            error("State was created with First, but Rest was not called",
...
def Rest(var, defn):
    ...
    if not isinstance(var, First):
        error(str(var) + " in Rest should be an instance of First.")
    var.rest = defn
...
Rest(indexf, Ufmodf(indexf + phase_incr, Ugen("TABLE_LEN")))
...
Rest(phase, Ufmod(phase + freq / SR, 1))
